...
import {
  ...
  setDoc,
  deleteDoc,
  orderBy, // Necesario para ordenar mensajes
...
const App = () => {
  ...
  const [appId, setAppId] = useState(null); // Estado para almacenar el appId
    ...
    const initializeFirebase = async () => {
      try {
        ...
        const isConfigComplete = requiredConfigKeys.every(key => 
          ...
          !firebaseConfigToUse[key].includes("TU_API_KEY_DE_FIREBASE") && // Evitar marcadores de posición
  ...
  return (
    <FirebaseContext.Provider value={{ db, auth, user, userId, appId }}> {/* Pasar appId en el contexto */}
...
const MainApp = ({ showSimulatedNotification }) => {
  ...
  const { auth, userId, appId } = useContext(FirebaseContext); // Ahora userId y appId se desestructuran del contexto
  ...
  return (
    <div className="flex flex-col min-h-screen pt-4 sm:pt-10 bg-gray-100"> {/* Ajuste de padding superior */}
      <div className="bg-blue-600 p-4 sm:p-6 flex flex-col items-center rounded-b-3xl shadow-xl"> {/* Ajuste de padding */}
        <h1 className="text-2xl sm:text-3xl font-bold text-white mb-1 sm:mb-2">Cambiazo</h1> {/* Nuevo nombre de la aplicación */}
        {/* El ID de usuario ya no se muestra aquí */}
...
const PostProductForm = ({ onSubmit, onCancel }) => {
  ...
  const handleSubmit = () => {
    ...
    if (name && price && description && city && category && condition) {
      onSubmit({ name, price, description, city, category, condition, imageUrls }); // No pasar 'contact'
      setMessage('');
    } else {
...
const PostJobForm = ({ onSubmit, onCancel }) => {
  ...
  const handleSubmit = () => {
    ...
    if (title && description && city) {
      onSubmit({ title, description, city }); // No pasar 'contact'
      setMessage('');
    } else {
...
const ReviewForm = ({ sellerId, productId, onClose, showSimulatedNotification }) => {
  ...
  const handleSubmitReview = async () => {
    if (!db || !userId || !sellerId || !rating || !appId) { // Asegurarse de que appId esté disponible
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl h-5/6 flex flex-col p-4 sm:p-6 overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto pr-2"> {/* Agregado pr-2 para espacio del scrollbar */}
          ...
          <ul className="list-disc list-inside mb-4 text-sm sm:text-base text-gray-700 ml-4">
            ...
            <li>Para proteger los derechos, la propiedad o la seguridad de Cambiazo, nuestros usuarios o el público.</li>
          </ul>
